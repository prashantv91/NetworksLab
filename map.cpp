...
void Map::draw_map(Mask *mask, int posx, int posy)
{
    ...
    else
    {
        ...
        REP(x, MAP_MAXX)
            REP(y, MAP_MAXY)
                // character can only be seen if he is within range
                if((mask -> getvis(x, y)) && (map[x][y] == '.' || map[x][y] == '#' || (abs(x-posx) <= DIST_VIS && abs(y-posy) <= DIST_VIS)))
                        //x and y interchanged due to differing conventions.
                        textprintf_ex(screen, font, (y+1)*10, (x+1)*10, makecol(255, 100, 200), -1, "%c", map[x][y]);  
                else if(mask -> getvis(x, y))
                        textprintf_ex(screen, font, (y+1)*10, (x+1)*10, makecol(255, 100, 200), -1, ".");  
